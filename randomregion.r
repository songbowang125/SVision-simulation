...
possiblevariants<-c('deletion', 'insertion', 'inversion', 'tandem duplication', 'inverted tandem duplication', 'perfect tandem repetition', 'approximate tandem repetition', 'translocation copy-paste', 'translocation cut-paste' ,  'reciprocal translocation', 'dispersed duplication', 'dispersed inverted duplication')
...
new_num = length(varwithproportions)
regions<-createRandomRegions(new_num, length.mean=opt$length, length.sd=opt$standarddev, genome=genome, mask=exclude, non.overlapping=TRUE)
...
for(i in (1:nrow(df))) {
  if (df$type[i] == 'deletion') {
  ...
  } else if (df$type[i] == 'dispersed duplication') {

    dis_roll = sample(500: 10000, 1)
    newregion<-createRandomRegions(1, length.mean=opt$length, length.sd=opt$standarddev, genome=genome, mask=newexclude, non.overlapping=TRUE)
    ...
    if (start >= df$start[i]) {
        start = df$start[i] + dis_roll
        end = start + region_len
    } else {
        end = df$start[i] - dis_roll
  ...
  } else if (df$type[i] == 'dispersed inverted duplication') {
